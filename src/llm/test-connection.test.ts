...
describe('testLlmConnection (openai-compatible) — error mapping', () => {
  ...
  test('api_key_env named but not set in server env → names the var', async () => {
    const r = await testLlmConnection({ engine: 'openai-compatible', base_url: 'https://x/v1', model: 'm', api_key_env: 'TEST_KEY' });
    expect(r.ok).toBe(false);
    expect(r.error).toMatch(/TEST_KEY/);
  });
  test('401 → auth hint', async () => {
    process.env.TEST_KEY = 'sk-bad';
